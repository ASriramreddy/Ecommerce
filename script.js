...
function renderCart() {
  ...
  cartItems.innerHTML = cart.length ? cart.map((item) => {
    ...
    return `<div class="cart-item"><div><h3>${product.name}</h3><p>${formatPrice(finalPrice(product))} each${discountFor(product) ? ` (${discountFor(product)}% off)` : ""}</p><small class="cart-stock">${stock} available · ${item.quantity} selected</small><div class="quantity-controls"><button type="button" data-change="${product.id}" data-amount="-1" aria-label="Decrease ${product.name} quantity">−</button><span>${item.quantity}</span><button type="button" data-change="${product.id}" data-amount="1" aria-label="Increase ${product.name} quantity" ${item.quantity >= stock ? "disabled" : ""}>+</button></div></div><button class="remove-item" type="button" data-remove="${product.id}">Remove</button></div>`;
